...
def rank_one_update_multivariable(f, vars, init_guess, tol=1e-6, max_iter=100):
    """
    Rank-One Update Algorithm for multivariable optimization with automatic gradient computation.
    ...
    # Symbolic variable for alpha (step size)
    ...
    for i in range(max_iter):
        ...
        #alpha_k = scipy.optimize.fmin(alpha_func,1);
        ...
        delta_diff = delta_k - G @ gamma_k
        G = G + np.outer(delta_diff, delta_diff) / (delta_diff.T @ gamma_k)
...
# Call Rank-One Update method
x_min, f_min = rank_one_update_multivariable(f, [x1, x2], init_guess)
